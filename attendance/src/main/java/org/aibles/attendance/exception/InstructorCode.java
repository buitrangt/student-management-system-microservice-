...
public class InstructorCode {
    ...
    // Các mã phản hồi mới
    public static final ResponseStatus COURSE_ALREADY_EXISTS =
            new ResponseStatus("COURSE_ALREADY_EXISTS", "Course already exists", HttpStatus.CONFLICT);

    public static final ResponseStatus STUDENT_ALREADY_EXISTS =
            new ResponseStatus("STUDENT_ALREADY_EXISTS", "Student already exists", HttpStatus.CONFLICT);

    public static final ResponseStatus INVALID_ATTENDANCE_STATUS =
            new ResponseStatus("INVALID_ATTENDANCE_STATUS", "Invalid attendance status", HttpStatus.BAD_REQUEST);
